...
import com.intravel.Activity.HotelListDetailsListActivity;
...
public class OAUTH extends Activity {
	...
	public boolean POST_AFTER_LOGIN=false;
	...
	private static final Uri CALLBACK_URI = Uri.parse("http://star-digital.co.uk/about-star-digital.html");
	...
	protected void onResume() {
		...
		if (uri != null && CALLBACK_URI.getScheme().equals(uri.getScheme())) {
			...
			Intent home = new Intent(this, HotelListDetailsListActivity.class); // Currently, how we get back to main activity. 
			...
			try {
				...
				home.putExtra(USER_TOKEN, token);
				home.putExtra(USER_SECRET, secret);
			} catch (OAuthMessageSignerException e) {
				e.printStackTrace();
			} catch (OAuthNotAuthorizedException e) {
				e.printStackTrace();
				...
				startActivity(home); // we either authenticated and have the extras or not, but we're going back
				finish();
				this.finish();
			}
		}
	...
	public static void saveAuthInformation(SharedPreferences settings, String token, String secret) 
	{
		...
		editor.putBoolean("POST_AFTER_LOGIN", true);
		editor.commit();
